...
import { approveCompany, rejectCompany } from "@/app/actions/admin-company";
...
export function CompanyRow({ row }: { row: CompanyRowData }) {
  ...
  const onApprove = () => {
    ...
    startTransition(async () => {
      const r = await approveCompany({ companyId: row.id });
  ...
  const onReject = () => {
    setError(null);
    if (reason.trim().length < 3) { setError("reason_too_short"); return; }
    startTransition(async () => {
      const r = await rejectCompany({ companyId: row.id, rejectionReason: reason });
      if (r.ok) router.refresh();
      else setError(r.reason ?? "generic");
    });
  };
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-muted truncate">
            {row.legalName}{row.country ? ` · ${row.country}` : ""} · @{row.ownerUsername}
          </div>
          {row.address && <div className="text-xs text-muted truncate">{row.address}</div>}
          {row.email && <div className="text-xs text-muted truncate">{row.email}</div>}
          {row.website && (
            <a href={row.website} target="_blank" rel="noopener noreferrer"
              className="text-xs text-accent break-all">{row.website}</a>
          )}
        </div>
        {row.status === "PENDING" && (
